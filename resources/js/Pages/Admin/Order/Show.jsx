                        ...
                        {order.payments && order.payments.length > 0 ? (
                            <table className="w-full text-sm">
                                <thead><tr className="border-b border-gray-100">
                                    ...
                                    <th className="text-center py-2 text-xs font-semibold text-gray-400 w-16">AKSI</th>
                                </tr></thead>
                                <tbody>
                                    {order.payments.map((p) => (
                                        <tr key={p.id} className="border-b border-gray-50 group">
                                            ...
                                            <td className="py-3 text-center">
                                                <button
                                                    onClick={() => {
                                                        if (confirm(`Hapus pembayaran ${formatPrice(p.amount)} via ${p.method}?`)) {
                                                            router.delete(`/admin/payments/${p.id}`, { preserveScroll: true });
                                                        }
                                                    }}
                                                    className="inline-flex items-center justify-center w-7 h-7 rounded-lg text-gray-300 hover:text-red-500 hover:bg-red-50 transition-all cursor-pointer opacity-0 group-hover:opacity-100"
